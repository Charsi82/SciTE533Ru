...
//std::wstring StringFromUTF8(const char* src, int cp)
//{
//	if (src && *src)
//		if (int len = ::MultiByteToWideChar(cp, 0, src, -1, NULL, 0))
//		{
//			std::unique_ptr<wchar_t[]> buff = std::make_unique<wchar_t[]>(len);
//			::MultiByteToWideChar(cp, 0, src, -1, buff.get(), len);
//			return buff.get();
//		}
//	return std::wstring();
//}
//
//std::string UTF8FromString(const std::wstring& src, int cp)
//{
//	if (int len = ::WideCharToMultiByte(cp, 0, src.data(), -1, NULL, 0, NULL, NULL))
//	{
//		std::unique_ptr<char[]> buff = std::make_unique<char[]>(len);
//		::WideCharToMultiByte(cp, 0, src.data(), -1, buff.get(), len, NULL, NULL);
//		return buff.get();
//	}
//	return  std::string();
//}
